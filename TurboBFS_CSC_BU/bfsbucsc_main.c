 ...
 *   TurboBFS_CSC_BU:bfsbucsc_main.c
 *
 *   This program:
 ...
 *   3) computes a sequential BFS for unweighted, and weighted
 *      graphs with sparse matrices in the CSC  format.            
 *   4) computes a single precision GPU-based parallel Bottom-Up
 *      BFS  for undirected and directed graphs with sparse 
 *      matrices in the CSC format.This type of BFS computes
 *      the S array to store the depth at which each vertex is 
 *      discovered 
 *   
 *   Compile: make with Makefile
 *
 *   Run with .sh file: ./run_bfsbucsc.sh  
 *   
...
int main(int argc, char *argv[]){
  ...
  printf("compute size of sparseadjacency matrix starting....\n ");
  ...
  **************************************************************************/
  ...
  /**************************************************************************
     compute sequential top-down BFS for unweighted graphs represented by sparse 
     adjacency matrices in the CSC format
  **************************************************************************/
  initial_t = get_time();
  if (seq){// CSC format
    bfs_seq_td_csc (CscA.IC,CscA.CP,S,sigma,r,nz,N);
    bfs_bu_seq_t = get_time()-initial_t;
    printf("bfs bu seq CSC time = %lfs\n", bfs_bu_seq_t);
  }

  /**************************************************************************
     compute GPU-based parallel bottom-up BFS for unweighted graphs represented 
     by sparse adjacency matrices in the CSC format         
  **************************************************************************/
  if (format == 0){// CSC(sc) format
    bfs_gpu_bu_csc_sc (CscA.IC,CscA.CP,S_hgpu,r,nz,N,repet,bbreak);
    if (seq){
      printf("\ncheck values of S_hgpu: ");
      S_check(S,S_hgpu,N);
    }
  }else if (format == 1){ // CSC(wa) format
    bfs_gpu_bu_csc_wa (CscA.IC,CscA.CP,S_hgpu,r,nz,N,repet,bbreak);
    if (seq){
      printf("\ncheck values of S_hgpu: ");
      S_check(S,S_hgpu,N);
    }
  ...
  **************************************************************************/
  ...
  int pt = 1;
  if (pt) {
    printf("\nscan=%2.3lfs,degree_t= %2.3lfs,transf_format_t=%2.3lfs,bfs_bu_seq_t=%2.3lfs\n",
	   scan_t,degree_t,transf_format_t,bfs_bu_seq_t);
  }
