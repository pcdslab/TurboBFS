 ...
 *   TurboBFS_COOC_TD:bfstdcooc_main.c
 *
 *   This program:
 ...
 *      graphs with sparse matrices in the COOC  format.            
 *   4) computes a single precision GPU-based parallel top-down BFS 
 *      for undirected and directed graphs represented by sparse adjacency
 *      matrices  in the COOC format.
 *   
 *   Compile: make with Makefile
 *
 *   Run with .sh file: ./run_bfstdcooc.sh 
 *
...
int main(int argc, char *argv[]){
  ...
  if (argc < 7){
    printf("number of arguments not valid, should be 7.\n");
  ...
  **************************************************************************/
  ...
  if (ug){//undirected graphs
    ...
    ************************************************************************/
    ...
    /************************************************************************
       compute CPLT, RPLT, CP and RP arrays for undirected graphs
    ************************************************************************/
    CP_RP_ug (out_degree,in_degree,degree,CPLT,RPLT,CP,RP,N);
    ...
    /************************************************************************/
    initial_t = get_time();
    SSCCOOC_uw (ICLT,CPLT,RPLT,RP,CP,CoocA.ICOOC,CoocA.JCOOC,N);    
  ...
  **************************************************************************/
  ...
  initial_t = get_time();
  if (seq){// COOC format
    bfs_seq_td_cooc (CoocA.ICOOC,CoocA.JCOOC,sigma,r,nz,N);
    bfs_td_seq_t = get_time()-initial_t;
    printf("bfs td seq COOC time = %lfs\n", bfs_td_seq_t);
  }
  //printf("\nsigma array\n");for (i=0; i<N; i++){printf("%d %lf\n", i,sigma[i]);}

  /**************************************************************************
     compute GPU-based parallel BFS for unweighted graphs represented by sparse
     adjacency matrices in the COOC format         
  **************************************************************************/
    bfs_gpu_td_cooc_sc (CoocA.ICOOC,CoocA.JCOOC,sigma_hgpu,r,nz,N,repet);
    if (seq){
  ...
  **************************************************************************/
  ...
  if (p) {printBFS(CoocA.ICOOC,CoocA.JCOOC,sigma,sigma_hgpu,nz,N);}
  int pt = 1;
  if (pt) {
