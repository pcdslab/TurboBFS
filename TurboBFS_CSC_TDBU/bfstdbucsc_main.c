 ...
 *   TurboBFS_CSC_TDBU:bfstdbucsc_main.c
 *
 *   This program:
 ...
 *   4) computes a single precision GPU-based parallel combined top-down/bottom-up BFS 
 *      for undirected and directed graphs represented by sparse  matrices in the CSC 
 *      format. This package implements the direction optimization algorithm switching
 *      from the top-down BFS to the bottom-up BFS when the frontier vector becomes dense.     
 *   
 *   Compile: make with Makefile
 *
 *   Run with .sh file: ./run_bfstdbucsc.sh  
 *   
 ...
 *   --bbreak: 1 to break the Bottom-Up BFS kernel, 0 otherwise
 *
 *   
 */
...
int main(int argc, char *argv[]){
  ...
  sigma = (int *) calloc(N,sizeof(*sigma));
  sigma_hgpu = (int *) calloc(N,sizeof(*sigma_hgpu));
  ...
  **************************************************************************/
  printf("computing sequential BFS starting:r=%d\n",r);
  initial_t = get_time();
  if (seq){
  ...
  /**************************************************************************
     compute GPU-based parallel top down/bottom up BFS for unweighted graphs
     represented by sparse adjacency matrices in the CSC format         
  **************************************************************************/
  if (format == 0){// CSC(sc) format
    bfs_gpu_tdbu_csc_sc (CscA.IC,CscA.CP,S_hgpu,sigma_hgpu,r,nz,N,repet,bbreak);
    if (seq){
      printf("\ncheck values of S_hgpu (sc): ");
      S_check(S,S_hgpu,N);
    }
  }else if (format == 1){ // CSC(wa) format
    bfs_gpu_tdbu_csc_wa (CscA.IC,CscA.CP,S_hgpu,sigma_hgpu,r,nz,N,repet,
                          bbreak);
    if (seq){
      printf("\ncheck values of S_hgpu (td-->bu): ");
      S_check(S,S_hgpu,N);
    } 
  }
  ...
  **************************************************************************/
  ...
  if (p) {printBFS(CscA.IC,CscA.CP,S,S_hgpu,sigma,nz,N);}
  int pt = 1;
  if (pt) {
    printf("\nscan=%2.3lfs,degree_t= %2.3lfs,transf-format_t=%2.3lfs,bfs_td_seq_t=%2.3lfs\n",
